...
.getRequestDispatcher(Constantes.VIEW_SIGNUP);
			...
			case Constantes.ACCION_MOSTRAR_REGENERAR_PASS:
				mostrarRecuperarPass(request, response);
				break;
			default:
...
.getRequestDispatcher(Constantes.VIEW_SIGNUP);
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			msg = null;
			request.setCharacterEncoding("UTF-8");
			pAccion = Integer.parseInt(request.getParameter("action"));
			switch (pAccion) {
			case Constantes.ACCION_REGISTRAR_USUARIO:
				...
				if (!modeloUsuario.checkUser(pNombre, pEmail)) { // Comprobamos
																				// si
																				// existe
																				// el
																				// usuario
...
.getRequestDispatcher(Constantes.VIEW_SIGNUP);
						...
						params.put("{url}", Constantes.URL_VALIDATE + pId
								+ "&email=" + pEmail);
						params.put(
								"{contenido}",
...
.getRequestDispatcher(Constantes.VIEW_SIGNUP);
			...
			case Constantes.ACCION_PASS_OLVIDADO:
				pEmail = request.getParameter("email-olvidado");
				usuario = (Usuario) modeloUsuario
						.getByEmail(pEmail);
				if (usuario != null) {
					usuario.setToken();
					...
					if (usuario.getValidado() == 1) {
						...
						params.put("{url}",
 Constantes.URL_PASS_OLVIDADO + pEmail + "&tkn=" + usuario.getToken());
						params.put(
								"{contenido}",
 "Si has olvidado tu contraseña haz click en el enlace de debajo para cambiarla.");
	...
	private void crearUsuario() {
		rol = new Rol("Usuario");
		rol.setId(2);
		usuario = new Usuario(pNombre, pEmail, pPass,
				rol);
	}
